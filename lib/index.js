...
class RequestResponseFilterStream extends stream.Transform {
    ...
    _transform (data, enc, next) {
        ...
        if (!config && data.tags && data.tags.length === 1 && data.tags[0] === 'content') return next();
    ...
    compile ({ viz, api, successes, warnings, errors }) {
...
exports.register = function (server, opts, next) {
    ...
    if (settingsPath) {
        server.log(['ent-hapi-redis-log', 'info'], `Request response logging is filtered using settings @request.auth.${settingsPath.join('.')}`);
        server.ext('onPostAuth', (req, reply) => {
            _.set(req.plugins, ['good', 'requestResponseFilter'], _.get(req.auth, settingsPath));
            reply.continue();
        });
